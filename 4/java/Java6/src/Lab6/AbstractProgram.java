...
class AbstractProgram implements Runnable {
    ...
    private final ExecutionState[] possibleNewStates = {ExecutionState.FATAL_ERROR,
            ExecutionState.RUNNING, ExecutionState.RUNNING, ExecutionState.RUNNING, ExecutionState.STOPPING};
    ...
    private void randomChangeState() {
        ...
        while (true) {
            ...
            try {
                Thread.sleep(randomChangeStateInterval);
            } catch (InterruptedException e) {
            ...
            synchronized (this) {
                ...
                System.out.println("Программа.Демон: ExecutionState --> " + state.toString());
                this.notify();
                if (state == ExecutionState.FATAL_ERROR || state == ExecutionState.STOPPING) {
                    break;
                }
            }
        }
