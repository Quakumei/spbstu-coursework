...
#include <list>
#include <map>
#include "../src/BidirectionalList.hpp"
...
TEST_CASE("BidirectionalList works as <list>", "[Unit]")
{
  REQUIRE(1 == 1);

  tampio::BidirectionalList< int > tamp_fl;
  std::list< int > stl_fl;
...
TEST_CASE("ForwardList iterator doing good!", "[Unit]")
{
  tampio::ForwardList< int > tamp_fl;
  std::forward_list< int > stl_fl;
  // Iteration
...
TEST_CASE("BiderectionalList iterator doing good!", "[Unit]")
{
  tampio::BidirectionalList< int > tamp_fl;
  std::list< int > stl_fl;
  // Iteration
  ...
  std::list< int >::iterator it_stl = stl_fl.begin();
  tampio::BidirectionalList< int >::Iterator it_tamp = tamp_fl.begin();
  for (int i = 0; i < 5; i++)
  {
    REQUIRE(*(it_stl++) == *(it_tamp++));
  }

  std::list< int >::iterator it_stl_bb = stl_fl.begin();
  tampio::BidirectionalList< int >::Iterator it_tamp_bb = tamp_fl.begin();
  for (int i = 0; i < 4; i++)
  {
    REQUIRE(*(++it_stl_bb) == *(++it_tamp_bb));
  }
  ...
  // first
  const int HOLY = 42;
  tamp_fl.insertBefore(tamp_fl.begin(), HOLY);
  stl_fl.insert(stl_fl.begin(), HOLY);
  it_stl = stl_fl.begin();
  it_tamp = tamp_fl.begin();
  REQUIRE(*(it_stl) == *(it_tamp));
  REQUIRE(HOLY == *(it_tamp));
  ...
  const int HOLY_2 = 43;
  // std::cout <<  "insert ready 0\n";
  tamp_fl.insertBefore(tamp_fl.end(), HOLY_2);
  REQUIRE(HOLY_2 == *(tamp_fl.beforeEnd()));
  // std::cout <<  "insert done 1\n";

  // it_stl_bb = stl_fl.before_begin();
  // it_tamp_bb = tamp_fl.beforeBegin();
  for (tampio::BidirectionalList< int >::Iterator i = tamp_fl.begin(); i != tamp_fl.end(); i++)
  {
    std::cout << ' ' << *i;
  }
  std::cout << '\n';
  std::cout << "delete2, commence\n";
  tamp_fl.deleteNode(tamp_fl.begin());
  std::cout << "delete2, successful\n";
  for (tampio::BidirectionalList< int >::Iterator i = tamp_fl.begin(); i != tamp_fl.end(); i++)
  {
  ...
  REQUIRE(0 == *(tamp_fl.beforeEnd()));
  std::cout << "assert success 2\n";
  std::cout << "delete3, commence\n";
  tamp_fl.deleteNode(++(++tamp_fl.begin()));
  std::cout << "delete3, successful\n";
  for (tampio::BidirectionalList< int >::Iterator i = tamp_fl.begin(); i != tamp_fl.end(); i++)
  {
  ...
  REQUIRE(1 == *(++(++tamp_fl.begin())));
  std::cout << "assert success 3";
  REQUIRE(--(++tamp_fl.begin()) == tamp_fl.begin());
  tampio::BidirectionalList< int >::Iterator rofl = tamp_fl.begin();
  REQUIRE(*(rofl++) == 4);
  REQUIRE(*(rofl++) == 3);
  REQUIRE(*(rofl++) == 1);
  REQUIRE(*(rofl) == 0);
  rofl = tamp_fl.beforeEnd();
  REQUIRE(*(rofl--) == 0);
  REQUIRE(*(rofl--) == 1);
  REQUIRE(*(rofl--) == 3);
  REQUIRE(*(rofl) == 4);
